...
num, pos, masses, uc, types, a = mm.makeMem(n1)
...
with open(velsfile, 'r') as fid: 
    ...
    ids = np.argwhere(types == 104) #atoms in this basis pos
...
#        dos[:,0] = dos[:,0]+vdos[0][:]
        ...
        for j in range(nk): #loop over all k points
            ...
            for l in range(nb): #loop over basis atoms
                ...
                tmpfft = np.fft.fft(tmp,axis=0)*(dt*dn) #*dt
